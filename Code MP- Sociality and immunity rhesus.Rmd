...
weight_dataset$Group1 <- factor(weight_dataset$Group, levels = c('S','HH','KK'))
...
mw_all_gd_pexp <- MCMCglmm(cbind(scale(spleen_wt), scale(liver_wt)) ~  -1 + 
                             trait:(scale(age) + I(age^2)) + 
                           trait:scale(body_wt) + trait:Grooming_deg + 
...
mw_all_ge_pexp <- MCMCglmm(cbind(scale(spleen_wt), scale(liver_wt)) ~  -1 + 
                        trait:(scale(age) + I(age^2)) + 
                        trait:scale(body_wt) + trait:(Grooming_eigen*Group) + 
                        trait:scale(perc.rank)  + trait:sex , 
                         random =~us(trait):id,
                         rcov=~ idh(trait):units,
                         family = c('gaussian','gaussian'),
...
mcbc_all_ge_pexp <- MCMCglmm(cbind(scale(WBCs),scale(RBC), scale(PLT)) ~ -1 + 
                           trait:(scale(age) + I(age^2)) + 
                           trait:scale(body_wt) + trait:Grooming_eigen +
                          trait:scale(perc.rank) + trait:sex + trait:Group, 
                         random =~us(trait):id,
                         rcov=~ idh(trait):units,
                         family = rep('gaussian',3),
...
rank_2 <- mw_all_gd$Sol[,'traitliver_wt:scale(perc.rank)']
gd_1 <- mw_all_gd$Sol[,'traitspleen_wt:scale(Grooming_deg)']
gd_2 <- mw_all_gd$Sol[,'traitliver_wt:scale(Grooming_deg)']
ge_1 <- mw_all_ge$Sol[,'traitspleen_wt:scale(Grooming_eigen)']
ge_2 <- mw_all_ge$Sol[,'traitliver_wt:scale(Grooming_eigen)']
...
plotdensities(distributions = distributions,
sublegend=sublegend, btylegend = "n",
globalmaxdensity = TRUE, violine = FALSE, xlim= c(-0.6,0.5),
xshift = c(0.138,0), yshift = c(0.1,0),cexsubl = 1, xlab = "Effect size", adjust = 1.5,
col = Dcolors, minden= 0.33, ylab="Probability density",
 cex.lab=1.3, cex.axis=1)
...
Social status effect on liver
--------------------

```{r results="hide"}

mw_all_ss <- MCMCglmm(cbind(scale(spleen_wt), scale(liver_wt)) ~ -1 + 
                              trait:(scale(age) + I(age^2)) + 
...
sl_all_fixed_eff1 <- list(mw_all_ss$Sol[,'traitspleen_wt:scale(age)'],
                        mw_all_ss$Sol[,'traitliver_wt:scale(age)'],
                        mw_all_ss$Sol[,'traitspleen_wt:I(age^2)'], 
                        mw_all_ss$Sol[,'traitliver_wt:I(age^2)'],
                        mw_all_ss$Sol[,'traitspleen_wt:scale(body_wt)'],
                        mw_all_ss$Sol[,'traitliver_wt:scale(body_wt)'],
                        mw_all_ss$Sol[,'traitspleen_wt:scale(perc.rank)'], 
                        mw_all_ss$Sol[,'traitliver_wt:scale(perc.rank)'],
                        mw_all_ss$Sol[,'traitspleen_wt:GroupKK'],
                        mw_all_ss$Sol[,'traitliver_wt:GroupKK'],
                        mw_all_ss$Sol[,'traitspleen_wt:GroupS'],
                        mw_all_ss$Sol[,'traitliver_wt:GroupS'],
                        mw_all_ss$Sol[,'traitspleen_wt:sexM'],
                        mw_all_ss$Sol[,'traitliver_wt:sexM'])

sl_all_table_fixed_eff1 <- data.frame(trait = c('Spleen','Liver',
                                          'Spleen','Liver',
                                          'Spleen','Liver',
                                          'Spleen','Liver', 
                                          'Spleen','Liver',
                            ...
                            parameter = c('Age','Age',
                                          ...
                                          'Group KK', 'Group S',
                                          'Group S', 'Sex','Sex'),
                           estimate = unlist(lapply(sl_all_fixed_eff1, mean)),
                           CI = unlist(lapply(sl_all_fixed_eff1, function(x){
                             paste0("[", round(HPDinterval(x)[1],2)," ; ", 
                                    round(HPDinterval(x)[2],2), "]")})))

sl_all_table_fixed_eff1
